...
def main():
    ...
    DATASET_FNAME = 'TransformedLinear Acceleration.csv'
    RESULT_FNAME = 'chapter5_result.csv'

    try:
        dataset = pd.read_csv(DATA_PATH / DATASET_FNAME)
        dataset.index = pd.to_datetime(dataset['Time (s)'], unit='ns')
    except IOError as e:
    ...
    if FLAGS.mode == 'final':
        ...
        dataset = clusteringNH.k_means_over_instances(dataset, ['x', 'y', 'x'], FLAGS.k, 'default', 50, 50)
        DataViz.plot_clusters_3d(dataset, ['x', 'y', 'z'], 'cluster', ['label'])
        DataViz.plot_silhouette(dataset, 'cluster', 'silhouette')
        util.print_latex_statistics_clusters(
            dataset, 'cluster', ['x', 'y', 'z'], 'label')
