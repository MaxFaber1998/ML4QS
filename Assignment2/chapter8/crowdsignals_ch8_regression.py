...
DATA_PATH = Path(f'{ROOT_DIR}/phyphox_exports/')
...
basic_features = ['gyr_x', 'gyr_y', 'gyr_z',
                  'lin_acc_x', 'lin_acc_y', 'lin_acc_z',
                  'mag_x', 'mag_y', 'mag_z',
                  'labelOnTable', 'labelSitting', 'labelWalking', 'labelStanding', 'labelWaving', 'labelEating']
pca_features = ['pca_1', 'pca_2', 'pca_3', 'pca_4', 'pca_5', 'pca_6', 'pca_7']
time_features = [name for name in dataset.columns if ('temp_' in name and not 'acc_' in name)]
freq_features = [name for name in dataset.columns if (('_freq' in name) or ('_pse' in name)) and not 'acc_' in name]
...
selected_features = ['temp_pattern_labelOnTable','labelOnTable', 'temp_pattern_labelOnTable(b)labelOnTable', 'cluster',
                     'pca_1_temp_mean_ws_300', 'pca_2_temp_mean_ws_300', 'pca_2', 'mag_y_temp_mean_ws_300', 'gyr_y_pse',
                     'gyr_x_pse']
...
#
# DataViz.plot_numerical_prediction_versus_real_dynsys_mo(train_X.index, train_y, test_X.index, test_y, output_sets, 0, 'acc_x')
#
# regr_train_y, regr_test_y = learner.dynamical_systems_model_ga(train_X, train_y, test_X, test_y, ['self.acc_x', 'self.acc_y', 'self.acc_z'],
#                                                      ['self.a * self.acc_x + self.b * self.acc_y', 'self.c * self.acc_y + self.d * self.acc_z', 'self.e * self.acc_x + self.f * self.acc_z'],
#                                                      ['self.acc_x', 'self.acc_y', 'self.acc_z'],
#                                                      ['self.a', 'self.b', 'self.c', 'self.d', 'self.e', 'self.f'],
...
#
# regr_train_y, regr_test_y = learner.dynamical_systems_model_sa(train_X, train_y, test_X, test_y, ['self.acc_x', 'self.acc_y', 'self.acc_z'],
#                                                      ['self.a * self.acc_x + self.b * self.acc_y', 'self.c * self.acc_y + self.d * self.acc_z', 'self.e * self.acc_x + self.f * self.acc_z'],
#                                                      ['self.acc_x', 'self.acc_y', 'self.acc_z'],
#                                                      ['self.a', 'self.b', 'self.c', 'self.d', 'self.e', 'self.f'],
#                                                      max_generations=10, per_time_step=True)
#
# DataViz.plot_numerical_prediction_versus_real(train_X.index, train_y['acc_x'], regr_train_y['acc_x'], test_X.index, test_y['acc_x'], regr_test_y['acc_x'], 'acc_x')
